...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    description:
      response?.contractURIMetadata?.description || "Unlock smarter monetization for your content.",
    keyword:
      response?.contractURIMetadata?.keyword ||
      `${response?.contractURIMetadata?.name} monetization, ${response?.contractURIMetadata?.name} ads`
  };
}
...
export default async function IframePage(req) {
  ...
  const flatAdList = response?.nftContract?.tokens.length
    ? [...response.nftContract.tokens.map(transformToAd)].sort((a, b) => a.tokenId - b.tokenId)
    : [];
